...
def env := Lampe.Env.mk [«bar», «square», «mtree_recover», «sgn0», «RC», «SIGMA», «as_array», «rotate_left», «main», «to_le_bits», «rl», «weird_eq_witness», «sbox», «from_le_bytes», «weird_assert_eq», «to_le_bytes», «permute»] [impl_405]