...
def env := Lampe.Env.mk [«as_array», «main», «from_le_bytes», «to_le_bytes», «rl», «sgn0», «bar», «rotate_left», «RC», «mtree_recover», «permute», «weird_assert_eq», «SIGMA», «sbox», «weird_eq_witness», «to_le_bits», «square»] [impl_405]