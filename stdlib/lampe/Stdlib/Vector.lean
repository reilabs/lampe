...
-- The beta.25 stdlib env is considerably larger (tuple impls up to arity 12,
-- new integer/internal modules), and `vector` sits near the end of the env
-- concatenation that `enter_decl` has to reduce through.
...
theorem append_spec {p T a b}
  : STHoare p env ⟦⟧
    («std-1.0.0-beta.25::vector::append».call h![T] h![a, b])
    (fun r => r = a ++ b) := by
  enter_decl
  steps
  step_as ([self ↦ ⟨T.vector, a⟩]) (fun _ => [self ↦ ⟨T.vector, a ++ b⟩])
  · steps

    loop_inv nat fun i hlo hhi => [self ↦ ⟨T.vector, a ++ (b.take i)⟩]
    · simp
    · simp
    · intro cond hlo hhi
      steps
...
theorem as_array_spec {p T N input}
  : STHoare p env ⟦⟧
    («std-1.0.0-beta.25::vector::as_array».call h![T, N] h![input])
...
theorem map_spec {T U Env p f fb l}
  ...
  : STHoare p env
    (Inv [] [] ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::map».call h![T, U, Env] h![l, f])
    (fun v => Inv l v ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
  step_as ([ret ↦ ⟨U.vector, []⟩] ⋆ [λf ↦ fb] ⋆ Inv [] [])
    (fun _ => ∃∃v, [ret ↦ ⟨U.vector, v⟩] ⋆ [λf ↦ fb] ⋆ Inv l v)
  · steps
    loop_inv nat fun i _ _ => ∃∃v, [ret ↦ ⟨U.vector, v⟩] ⋆ [λf ↦ fb] ⋆ Inv (l.take i) v
    · sl
    · simp
    · intro i ihl ihg
      steps
...
theorem mapi_spec {T U Env p f fb l}
  ...
  : STHoare p env
    (inv [] [] ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::mapi».call h![T, U, Env] h![l, f])
    (fun v => inv l v ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
  step_as ([ret ↦ ⟨U.vector, []⟩] ⋆ [index ↦ ⟨.u 32, 0⟩] ⋆ [λf ↦ fb] ⋆ inv [] [])
    (fun _ => ∃∃v, [ret ↦ ⟨U.vector, v⟩] ⋆ [index ↦ ⟨.u 32, l.length⟩] ⋆ [λf ↦ fb] ⋆ inv l v)
  · steps
    loop_inv nat fun i _ _ =>
      ∃∃v, [ret ↦ ⟨U.vector, v⟩] ⋆ [index ↦ ⟨.u 32, i⟩] ⋆ [λf ↦ fb] ⋆ (inv (l.take i) v)
    · sl
    · simp
    · intro i hlo hhi
      steps
...
theorem for_each_spec {T Env p f fb l}
  ...
  : STHoare p env
    (Inv [] ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::for_each».call h![T, Env] h![l, f])
...
theorem for_eachi_spec {T Env p f fb l}
  ...
  : STHoare p env
    (inv [] ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::for_eachi».call h![T, Env] h![l, f])
    (fun _ => inv l ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
...
theorem fold_spec {p T U Env l a f fb}
  ...
  : STHoare p env
    (inv [] a ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::fold».call h![T, U, Env] h![l, a, f])
...
theorem reduce_spec {p T Env l f fb}
  ...
  : STHoare p env
    ((inv [] l[0]) ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::reduce».call h![T, Env] h![l, f])
...
theorem filter_spec {p T Env l f fb}
  ...
  : STHoare p env
    (inv [] [] ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::filter».call h![T, Env] h![l, f])
    (fun r => (inv l r) ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
  step_as ([ret ↦ ⟨T.vector, []⟩] ⋆ [λf ↦ fb] ⋆ (inv [] []))
    (fun _ => ∃∃v, [ret ↦ ⟨T.vector, v⟩] ⋆ [λf ↦ fb] ⋆ (inv l v))
  · steps
    loop_inv nat fun i _ _ => ∃∃v, [ret ↦ ⟨T.vector, v⟩] ⋆ [λf ↦ fb] ⋆ (inv (l.take i) v)
    · sl
    · simp
    · intro i hlo hhi
      steps
...
theorem join_spec {p T a s}
  ...
  : STHoare p env ⟦⟧
    («std-1.0.0-beta.25::vector::join».call h![T] h![a, s])
...
theorem all_spec {p T Env l f fb}
  ...
  : STHoare p env
    ((inv [] true) ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::all».call h![T, Env] h![l, f])
    (fun r => (inv l r) ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
...
theorem any_spec {p T Env l f fb}
  ...
  : STHoare p env
    ((inv [] false) ⋆ [λf ↦ fb])
    («std-1.0.0-beta.25::vector::any».call h![T, Env] h![l, f])
    (fun r => (inv l r) ⋆ [λf ↦ fb]) := by
  enter_decl
  steps
