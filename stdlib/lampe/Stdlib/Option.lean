...
def from_option {p t}: Option (t.denote p) -> («struct#std::option::Option».tp h![t] |>.denote p)
| none => (false, Lampe.Tp.zero p t, ())
| some val => (true, val, ())

def to_option {p t}: («struct#std::option::Option».tp h![t] |>.denote p) -> Option (t.denote p)
| (false, _, _) => none
| (true, val, _) => some val
...
lemma some_spec {p T v} : Lampe.STHoare p env ⟦⟧ («std::option::Option::some».call h![T] h![v]) (fun r => r = from_option (some v)) := by
  enter_decl
  steps
  assumption

lemma is_none_spec {p T v} : Lampe.STHoare p env ⟦⟧ («std::option::Option::is_none».call h![T] h![from_option v]) (fun r => r = v.isNone) := by
  enter_decl
  steps
  subst_vars
  cases v <;> rfl
...
lemma unwrap_spec {p T v} : Lampe.STHoare p env ⟦⟧ («std::option::Option::unwrap».call h![T] h![from_option v]) (fun r => ∃h, r = v.get h) := by
  enter_decl
  steps
  cases v
  · subst_vars
...
lemma map_none_spec {p T U E f} : Lampe.STHoare p env ⟦⟧ («std::option::Option::map».call h![T, U, E] h![from_option none, f]) (fun r => r = from_option (none: Option (U.denote p))) := by
  enter_decl
  steps
...
lemma map_pure_spec {p T U E f fb v f_emb}
  ...
  Lampe.STHoare p env [λf ↦ fb] («std::option::Option::map».call h![T, U, E] h![from_option v, f]) (fun r => r = from_option (v.map f_emb)) := by
  cases v
  · steps [map_none_spec]
    assumption
