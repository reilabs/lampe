...
noir_def std::hash::poseidon2::Poseidon2::absorb<>(self: & std::hash::poseidon2::Poseidon2<>, input: Field) -> Unit := {
  ...
  if (#_uEq returning bool)((#_readRef returning std::hash::poseidon2::Poseidon2<>)(self).2, (RATE<> as λ() -> u32)()) then {
    ...
    {
      let (i_2535: Unit) = (#_readRef returning std::hash::poseidon2::Poseidon2<>)(self).2;
      (((*self: std::hash::poseidon2::Poseidon2<>).0: Array<Field, 3: u32>)[i_2535]: Field) = input;
      #_skip
    };
...
noir_def std::hash::poseidon2::Poseidon2::hash_internal<N: u32>(input: Array<Field, N: u32>, in_len: u32, is_variable_length: bool) -> Field := {
  ...
  for i in (0: u32) .. (std::array::len<> as λ(Array<Field, N: u32>) -> u32)(input) do {
...
noir_trait_impl[impl_0]<> std::hash::Hasher<> for std::hash::poseidon2::Poseidon2Hasher<> where [] := {
  noir_def finish<>(self: std::hash::poseidon2::Poseidon2Hasher<>) -> Field := {
    let (iv: Field) = (#_fMul returning Field)((#_cast returning Field)((std::slice::len<> as λ(Slice<Field>) -> u32)(self.0)), (18446744073709551616: Field));
    let mut (sponge: std::hash::poseidon2::Poseidon2<>) = (std::hash::poseidon2::Poseidon2::new<> as λ(Field) -> std::hash::poseidon2::Poseidon2<>)(iv);
    for i in (0: u32) .. (std::slice::len<> as λ(Slice<Field>) -> u32)(self.0) do {
  ...
  noir_def write<>(self: & std::hash::poseidon2::Poseidon2Hasher<>, input: Field) -> Unit := {
    ((*self: std::hash::poseidon2::Poseidon2Hasher<>).0: Slice<Field>) = (std::slice::push_back<> as λ(Slice<Field>, Field) -> Slice<Field>)((#_readRef returning std::hash::poseidon2::Poseidon2Hasher<>)(self).0, input);
    #_skip
  };
}
