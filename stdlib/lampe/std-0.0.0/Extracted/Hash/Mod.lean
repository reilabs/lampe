...
noir_def hash::hash_to_field<>(inputs: Slice<Field>) -> Field := {
  let mut (sum: Field) = (0: Field);
  {
    let (ζi0: Slice<Field>) = inputs;
    for ζi1 in (0: u32) .. (std::slice::len<> as λ(Slice<Field>) -> u32)(ζi0) do {
      let (input: Field) = (#_sliceIndex returning Field)(ζi0, (#_cast returning u32)(ζi1));
      {
        let (input_bytes: Array<u8, 32: u32>) = (std::field::to_le_bytes<32: u32> as λ(Field) -> Array<u8, 32: u32>)(input);
        sum = (#_fAdd returning Field)(sum, (field::bytes32_to_field<> as λ(Array<u8, 32: u32>) -> Field)((hash::blake2s<32: u32> as λ(Array<u8, 32: u32>) -> Array<u8, 32: u32>)(input_bytes)));
        #_skip
      }
    };
    #_skip
...
noir_trait_impl[impl_17]<N: u32, T: Type> std::hash::Hash<> for Array<T, N: u32> where [T: std::hash::Hash<>] := {
  noir_def hash<H: Type>(self: Array<T, N: u32>, state: & H) -> Unit := {
    ...
    for ζi1 in (0: u32) .. (std::array::len<> as λ(Array<T, N: u32>) -> u32)(ζi0) do {
...
noir_trait_impl[impl_18]<T: Type> std::hash::Hash<> for Slice<T> where [T: std::hash::Hash<>] := {
  noir_def hash<H: Type>(self: Slice<T>, state: & H) -> Unit := {
    ((u32 as std::hash::Hash<>)::hash<H> as λ(u32, & H) -> Unit)((std::slice::len<> as λ(Slice<T>) -> u32)(self), state);
    {
      let (ζi0: Slice<T>) = self;
      for ζi1 in (0: u32) .. (std::slice::len<> as λ(Slice<T>) -> u32)(ζi0) do {
...
noir_def hash::assert_pedersen<>() -> Unit := {
  ...
  (#_assert returning Unit)(((std::embedded_curve_ops::EmbeddedCurvePoint<> as Eq<>)::eq<> as λ(std::embedded_curve_ops::EmbeddedCurvePoint<>, std::embedded_curve_ops::EmbeddedCurvePoint<>) -> bool)((hash::pedersen_commitment_with_separator<10: u32> as λ(Array<Field, 10: u32>, u32) -> std::embedded_curve_ops::EmbeddedCurvePoint<>)((#_mkArray returning Array<Field, 10: u32>)((1: Field), (2: Field), (3: Field), (4: Field), (5: Field), (6: Field), (7: Field), (8: Field), (9: Field), (10: Field)), (10: u32)), (#_makeData returning std::embedded_curve_ops::EmbeddedCurvePoint<>)((-311556882567474412576811669014419164866085063813982345701073780395111226613: Field), (-163948955461070038478898748306472919392980231474444415480716967232318545340: Field), #_false)));
  #_skip
}


def Hash.Mod.env : Env := Env.mk
  [«hash::sha256_compression», «hash::keccakf1600», «hash::keccak::keccakf1600», «hash::blake2s», «hash::blake3», «hash::__blake3», «hash::pedersen_commitment», «hash::pedersen_commitment_with_separator», «hash::pedersen_hash», «hash::pedersen_hash_with_separator», «hash::derive_generators», «hash::__derive_generators», «hash::from_field_unsafe», «hash::hash_to_field», «hash::poseidon2_permutation», «hash::assert_pedersen»]
