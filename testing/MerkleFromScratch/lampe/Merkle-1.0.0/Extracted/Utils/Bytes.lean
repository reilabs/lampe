...
noir_def «Merkle-1.0.0»::utils::bytes::to_le_bytes<>(self: Field) -> Array<u8, 32: u32> := {
  let bits = («Merkle-1.0.0»::utils::bits::to_le_bits<> as λ(Field) -> Array<u1, 256: u32>)(self);
...
noir_def «Merkle-1.0.0»::utils::bytes::from_le_bytes<>(bytes: Array<u8, 32: u32>) -> Field := {
  let mut v = (1: Field);
...
def «Merkle-1.0.0».Utils.Bytes.env : Env := Env.mk
  [«Merkle-1.0.0::utils::bytes::to_le_bytes», «Merkle-1.0.0::utils::bytes::from_le_bytes»]
  []
