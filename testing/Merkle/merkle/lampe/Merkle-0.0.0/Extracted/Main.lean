...
noir_def «Merkle-0.0.0»::mtree_recover<H: Type, N: u32>(idx: Array<bool, N: u32>, p: Array<Field, N: u32>, item: Field) -> Field := {
  ...
  for i in (0: u32) .. uConst!(N: u32) do {
    ...
    if dir then {
      curr_h = ((H as «hasher-0.0.0»::BinaryHasher<Field>)::hash<> as λ(Field, Field) -> Field)(sibling_root, curr_h);
      #_skip
    } else {
      curr_h = ((H as «hasher-0.0.0»::BinaryHasher<Field>)::hash<> as λ(Field, Field) -> Field)(curr_h, sibling_root);
      #_skip
    }
  };
  curr_h
