...
`Scalar.Canonical` machinery introduced for the Pedersen wrapper
...
/-! ### `Scalar.Canonical` holds on `Scalar.canonicalDecomp` -/

/-- `Scalar.canonicalDecomp 0 = (0, 0)` and is canonical. -/
example : Scalar.Canonical (Scalar.canonicalDecomp (0 : Fp P)) := by
  native_decide

/-- `Scalar.canonicalDecomp 1 = (1, 0)` and is canonical. -/
example : Scalar.Canonical (Scalar.canonicalDecomp (1 : Fp P)) := by
  native_decide

/-- `Scalar.canonicalDecomp (p - 1) = (plo - 1, phi)` and is canonical (high
limb is `phi`, just below the `2^126` bound; low limb is `plo - 1`,
just below the `2^128` bound). -/
example : Scalar.Canonical (Scalar.canonicalDecomp (-1 : Fp P)) := by
  native_decide

/-- `Scalar.canonicalDecomp (pow128) = (0, 1)` — boundary between low- and
high-limb regimes. -/
example :
    Scalar.Canonical (Scalar.canonicalDecomp ((Lampe.pow128 : Nat) : Fp P)) := by
  native_decide

/-- `Scalar.canonicalDecomp (pow128 - 1) = (pow128 - 1, 0)` — largest pure-low
value. -/
example :
    Scalar.Canonical (Scalar.canonicalDecomp ((Lampe.pow128 - 1 : Nat) : Fp P)) := by
  native_decide

/-- `Scalar.canonicalDecomp (pow128 * phi) = (0, phi)` — top of the
...
/-- On `f = 0`, `Scalar.canonicalDecomp` picks the `(0, 0)` witness, not the
arithmetically-equivalent `(plo, phi)` witness. This is the
counterexample to "any canonical-range decomposition of a field
element is unique" — the `Scalar.canonicalDecomp_unique` lemma's
`hdisj` hypothesis breaks the tie. -/
example :
    (Scalar.lo (Scalar.canonicalDecomp (0 : Fp P))).val = 0 ∧
    (Scalar.hi (Scalar.canonicalDecomp (0 : Fp P))).val = 0 := by
  native_decide

/-- The spurious `(plo, phi)` witness is **not** what `Scalar.canonicalDecomp`
returns on `f = 0`, even though it satisfies the limb identity
`plo + 2^128 · phi = p ≡ 0 (mod p)`. -/
example :
    (Scalar.lo (Scalar.canonicalDecomp (0 : Fp P))).val ≠ Lampe.Crypto.Bn254.plo ∨
    (Scalar.hi (Scalar.canonicalDecomp (0 : Fp P))).val ≠ Lampe.Crypto.Bn254.phi := by
  native_decide
