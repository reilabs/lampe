...
namespace Lampe.Builtin.Poseidon2

/-- Internal diagonal matrix constants for Poseidon2 BN254 t=4. -/
def internalDiag : Array Nat := #[
...
/-- Round constants for the 8 external (full) rounds.
    Layout: rounds 0..3 then rounds 60..63, each with 4 constants. Total: 32 values. -/
def externalRC : Array Nat := #[
  -- Round 0
...
/-- Round constants for the 56 internal (partial) rounds.
    One constant per round (applied to state[0] only). -/
def internalRC : Array Nat := #[
...
/-- S-box: x^5 in Fp. -/
def sbox {p : Prime} (x : Fp p) : Fp p :=
  let x2 := x * x
  x2 * x2 * x

/-- External 4×4 MDS matrix multiplication (Barretenberg optimized form).
    M_E produces: [5a+7b+c+3d, 4a+6b+c+d, a+3b+5c+7d, a+b+4c+6d] -/
def matMulExternal {p : Prime} (s : List.Vector (Fp p) 4) : List.Vector (Fp p) 4 :=
  let a := s.get ⟨0, by omega⟩
  let b := s.get ⟨1, by omega⟩
  let c := s.get ⟨2, by omega⟩
  let d := s.get ⟨3, by omega⟩
  let t0 := a + b
  let t1 := c + d
  let t2 := b + b + t1
  let t3 := d + d + t0
...
/-- Internal matrix multiplication: state[i] = state[i] * diag[i] + sum(state). -/
def matMulInternal {p : Prime} (s : List.Vector (Fp p) 4) : List.Vector (Fp p) 4 :=
  let s0 := s.get ⟨0, by omega⟩
  let s1 := s.get ⟨1, by omega⟩
  let s2 := s.get ⟨2, by omega⟩
  let s3 := s.get ⟨3, by omega⟩
  let sum := s0 + s1 + s2 + s3
  ⟨[s0 * ↑(internalDiag[0]!) + sum,
    s1 * ↑(internalDiag[1]!) + sum,
    s2 * ↑(internalDiag[2]!) + sum,
    s3 * ↑(internalDiag[3]!) + sum], rfl⟩

/-- One full (external) round: add constants + full S-box + external matrix.
    `rcIdx` is the index into `externalRC` (0..7), mapping to 4 consecutive constants. -/
def fullRound {p : Prime} (rcIdx : Nat) (s : List.Vector (Fp p) 4) : List.Vector (Fp p) 4 :=
  let base := rcIdx * 4
  let s : List.Vector (Fp p) 4 :=
    ⟨[s.get ⟨0, by omega⟩ + ↑(externalRC[base]!),
      s.get ⟨1, by omega⟩ + ↑(externalRC[base + 1]!),
      s.get ⟨2, by omega⟩ + ↑(externalRC[base + 2]!),
      s.get ⟨3, by omega⟩ + ↑(externalRC[base + 3]!)], rfl⟩
  let s : List.Vector (Fp p) 4 :=
    ⟨[sbox (s.get ⟨0, by omega⟩), sbox (s.get ⟨1, by omega⟩),
      sbox (s.get ⟨2, by omega⟩), sbox (s.get ⟨3, by omega⟩)], rfl⟩
  matMulExternal s

/-- One partial (internal) round: add constant to s[0] + S-box on s[0] + internal matrix.
    `rcIdx` is the index into `internalRC` (0..55). -/
def partialRound {p : Prime} (rcIdx : Nat) (s : List.Vector (Fp p) 4) : List.Vector (Fp p) 4 :=
  let s0 := s.get ⟨0, by omega⟩ + ↑(internalRC[rcIdx]!)
  let s : List.Vector (Fp p) 4 :=
    ⟨[sbox s0, s.get ⟨1, by omega⟩, s.get ⟨2, by omega⟩, s.get ⟨3, by omega⟩], rfl⟩
  matMulInternal s

/-- The full Poseidon2 permutation for t=4 with BN254 parameters.
    Structure: initial M_E, 4 full rounds, 56 partial rounds, 4 full rounds. -/
def permute {p : Prime} (state : List.Vector (Fp p) 4) : List.Vector (Fp p) 4 :=
  -- Initial external linear layer
  let s := matMulExternal state
  -- First 4 full rounds (externalRC indices 0..3)
  let s := fullRound 0 s
  let s := fullRound 1 s
  let s := fullRound 2 s
  let s := fullRound 3 s
  -- 56 partial rounds (internalRC indices 0..55)
  let s := List.range 56 |>.foldl (fun acc i => partialRound i acc) s
  -- Last 4 full rounds (externalRC indices 4..7)
  let s := fullRound 4 s
  let s := fullRound 5 s
  let s := fullRound 6 s
  let s := fullRound 7 s
