...
| .ref _ => { val := 0 }
| .tuple (some name) (a :: as) => @NamedTuple.mk name _ (tpZeroed a, (tpZeroed (.tuple none as)))
| .tuple (some name) [] => @NamedTuple.mk name _ ()
| .tuple none (a :: as) => (tpZeroed a, tpZeroed (.tuple none as))
| .tuple none [] => ()
...
- If `tp` is a tuple, this evaluates to a tuple with all fields zeroed.
