...
@[reducible]
def replaceSlice' (s : Tp.denote p $ CTp.slice tp) (i : Fin s.length) (v : Tp.denote p tp) : Tp.denote p $ CTp.slice tp :=
...
def mkSlice (n : Nat) := newGenericPureBuiltin
  (fun (argTps, tp) => ⟨argTps, (CTp.slice tp)⟩)
  (fun (argTps, tp) args => ⟨argTps = List.replicate n (.concrete tp),
    fun h => HList.toList (rep := Tp.denote _) args h⟩)
...
def sliceIndex := newGenericPureBuiltin
  (fun tp => ⟨[CTp.slice tp, CTp.u 32], tp⟩)
...
def slicePushBack := newGenericPureBuiltin
  (fun tp => ⟨[CTp.slice tp, tp], CTp.slice tp⟩)
  (fun _ h![l, e] => ⟨True,
    fun _ => l ++ [e]⟩)
...
def sliceInsert := newGenericPureBuiltin
  (fun tp => ⟨[CTp.slice tp, CTp.u 32, tp], CTp.slice tp⟩)
...
def slicePopFront := newGenericPureBuiltin
  (fun tp => ⟨[CTp.slice tp], CTp.tuple none [tp, CTp.slice tp]⟩)
  (fun _ h![l] => ⟨l ≠ [],
...
def slicePopBack := newGenericPureBuiltin
  (fun tp => ⟨[CTp.slice tp], CTp.tuple none [CTp.slice tp, tp]⟩)
  (fun _ h![l] => ⟨l ≠ [],
...
def sliceRemove := newGenericPureBuiltin
  (fun tp => ⟨[CTp.slice tp, CTp.u 32], CTp.tuple none [CTp.slice tp, tp]⟩)
  (fun _ h![l, i] => ⟨i.toNat < l.length,
