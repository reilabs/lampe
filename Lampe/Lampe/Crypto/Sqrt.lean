...
`sqrt : Fp p → Bool × Fp p` for `p` carrying the BN254 scalar-prime
instance `[Lampe.Crypto.Bn254.Prime p]`. Returns `(true, y)` when
`y * y = a`, `(false, _)` otherwise.

Plain Tonelli–Shanks specialised to BN254 (where `r - 1 = 2^28 · q'`
with `q'` odd). Proved correct: TS produces a square root on
QR inputs; the Euler gate short-circuits to `(false, 0)` on
non-residues.
...
`z_nonresidue` verification below) uses this so the kernel sees an
...
`Fp p := ZMod p.natVal`, and `z_nonresidue` (which doesn't have a
...
theorem z_nonresidue :
  ...
  rw [← fpowR_eq_pow (z : ZMod r_scalar) _ 512
...
`fuel`. The outer fuel is at most `s = 28` (each iteration strictly
...
def findOrder {p : Prime} (acc : Fp p) (i M fuel : Nat) : Nat :=
  match fuel with
  | 0 => M
  | fuel + 1 =>
      if i ≥ M then M
...
`M`, so `s = 28` iterations suffice. -/
def tonelliLoop {p : Prime} (a : Fp p) (fuel : Nat) (M : Nat) (c t R : Fp p) : Fp p :=
  match fuel with
  | 0 => R
  | fuel + 1 =>
      if t = 1 then R
...
`tonelliLoop` for `s = 28` iterations. -/
def tonelliCandidate {p : Prime} [Lampe.Crypto.Bn254.Prime p] (a : Fp p) : Fp p :=
  let c0 : Fp p := fpowR ((z : Fp p)) qPrime 512
  let t0 : Fp p := fpowR a qPrime 512
  let R0 : Fp p := fpowR a ((qPrime + 1) / 2) 512
  tonelliLoop a s s c0 t0 R0
...
non-quadratic-residue inputs return `(false, 0)` in `O(log r)` field
...
underflows in `Nat`). -/
structure TSInv {p : Prime} (a : Fp p) (M : Nat) (c t R : Fp p) : Prop where
  Msq : R * R = a * t
  tpow : t ^ (2 ^ (M - 1)) = 1
  cpow : c ^ (2 ^ (M - 1)) = -1
  Mge : 1 ≤ M

/-! ### Initial state: TSInv holds at `(s, c0, t0, R0)`

We need:
- `R0 * R0 = a * t0`  where `R0 = a^((q'+1)/2)`, `t0 = a^q'`. Since
  `q' + 1` is even, `(q'+1)/2 + (q'+1)/2 = q'+1`, and `R0² = a^(q'+1) = a · a^q' = a · t0`.
- `t0^(2^(s-1)) = 1`. This is the Euler condition: `t0^(2^(s-1)) = a^(q' · 2^(s-1)) = a^((r-1)/2)`, which equals `1` because `a` is a QR.
- `c0^(2^(s-1)) = -1`. This is `z^(q' · 2^(s-1)) = z^((r-1)/2) = -1` by `z_nonresidue`.
-/

/-- Key identity: `q' · 2^(s - 1) = (r_scalar - 1) / 2`. -/
theorem qPrime_mul_two_pow_eq : qPrime * 2 ^ (s - 1) = (r_scalar - 1) / 2 := by
  unfold qPrime s r_scalar
  decide

/-- `z_nonresidue` transported to `Fp p` when `p.natVal = r_scalar`.

Done by viewing the goal as an equality between `Eq.mpr`-cast versions of
the corresponding `ZMod r_scalar` statement. -/
theorem z_nonresidue_fp {p : Prime} [hBn : Lampe.Crypto.Bn254.Prime p] :
    (z : Fp p) ^ ((r_scalar - 1) / 2) = -1 := by
  have hp : p.natVal = r_scalar := hBn.natVal_eq_r_scalar
  have hzn : (z : ZMod r_scalar) ^ ((r_scalar - 1) / 2) = -1 := z_nonresidue
...
theorem c0_inv_pow {p : Prime} [hBn : Lampe.Crypto.Bn254.Prime p] :
    ((z : Fp p) ^ qPrime) ^ (2 ^ (s - 1)) = -1 := by
  rw [← pow_mul, qPrime_mul_two_pow_eq]
  exact z_nonresidue_fp

theorem t0_pow_eq_one {p : Prime} [hBn : Lampe.Crypto.Bn254.Prime p]
    {a : Fp p} (ha : a ≠ 0) (hQR : ∃ y : Fp p, y * y = a) :
    (a ^ qPrime) ^ (2 ^ (s - 1)) = 1 := by
  rw [← pow_mul, qPrime_mul_two_pow_eq]
  exact euler_one_of_exists_sqrt ha hQR

theorem R0_sq {p : Prime} (a : Fp p) :
    (a ^ ((qPrime + 1) / 2)) * (a ^ ((qPrime + 1) / 2)) = a * (a ^ qPrime) := by
  rw [← pow_add]
  have heven : (qPrime + 1) % 2 = 0 := by
    have hq := qPrime_odd
    omega
  have hsum : (qPrime + 1) / 2 + (qPrime + 1) / 2 = qPrime + 1 := by
    have := Nat.div_add_mod (qPrime + 1) 2
    omega
  rw [hsum, pow_succ, mul_comm]

theorem tsInv_init {p : Prime} [hBn : Lampe.Crypto.Bn254.Prime p]
    {a : Fp p} (ha : a ≠ 0) (hQR : ∃ y : Fp p, y * y = a) :
    TSInv a s
      ((z : Fp p) ^ qPrime)
      (a ^ qPrime)
...
theorem findOrder_spec {p : Prime} (t : Fp p) :
...
theorem findOrder_init_spec {p : Prime} (t : Fp p) (M : Nat) (hM : 1 ≤ M) :
...
theorem findOrder_found_of_tsInv {p : Prime}
...
private theorem t_half_sq_eq_one {p : Prime} {t : Fp p} {j : Nat} (hj : 1 ≤ j)
...
private theorem sq_eq_one_imp_pm_one {p : Prime} {x : Fp p} (h : x ^ 2 = 1) :
    x = 1 ∨ x = -1 := by
...
private theorem t_half_eq_neg_one_of_min {p : Prime} {t : Fp p} {j : Nat} (hj : 1 ≤ j)
...
private theorem tsStep_b_eq {p : Prime} (c : Fp p) (M j : Nat) (hjlt : j < M) :
...
theorem tsStep_preserves {p : Prime}
...
The loop terminates within `s` iterations and produces `R` with `R*R = a`. -/

/-- If `TSInv a M c t R` and `t = 1`, then `R * R = a`. -/
theorem tsInv_terminal {p : Prime} {a : Fp p} {M : Nat} {c t R : Fp p}
...
theorem tonelliLoop_correct {p : Prime} {a : Fp p} :
...
theorem tonelliCandidate_sq {p : Prime} [Lampe.Crypto.Bn254.Prime p]
    {a : Fp p} (ha : a ≠ 0) (hQR : ∃ y : Fp p, y * y = a) :
    tonelliCandidate a * tonelliCandidate a = a := by
  unfold tonelliCandidate
  -- qPrime has ~226 bits; bridge via 2^254 to dodge the kernel's
  -- exponentiation threshold for `decide`.
  have h_bridge : (2 : Nat) ^ 254 ≤ 2 ^ 512 :=
    Nat.pow_le_pow_right (by omega) (by omega)
  have hq : qPrime < 2 ^ 512 :=
    lt_of_lt_of_le (by unfold qPrime; decide) h_bridge
  have hq1 : (qPrime + 1) / 2 < 2 ^ 512 :=
    lt_of_lt_of_le (by unfold qPrime; decide : (qPrime + 1) / 2 < 2 ^ 254) h_bridge
  rw [fpowR_eq_pow _ _ _ hq, fpowR_eq_pow _ _ _ hq, fpowR_eq_pow _ _ _ hq1]
  exact tonelliLoop_correct s s _ _ _ (tsInv_init ha hQR) (le_refl _)
