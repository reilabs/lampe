...
Instantiates `Lampe.Crypto.ShortWeierstrass` over `Fp Secp256k1.prime`
with curve coefficient `a = 0` (i.e. `y² = x³ + 7`). The curve math
lives in the shared module; this file only carries the curve constants,
the ECDSA-specific byte plumbing, and reference test vectors.
-/
...
/-- Generator x-coordinate (in `Fp Secp256k1.prime`). -/
def Gx : Fp Secp256k1.prime :=
  ((0x79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798 : Nat) : Fp _)

/-- Generator y-coordinate. -/
def Gy : Fp Secp256k1.prime :=
  ((0x483ADA7726A3C4655DA4FBFC0E1108A8FD17B448A68554199C47D08FFB10D4B8 : Nat) : Fp _)

/-- Curve `a` coefficient. -/
def curveA : Fp Secp256k1.prime := 0

/-- Curve `b` coefficient (for the Mathlib bridge: `y² = x³ + 7`). -/
def curveB : Fp Secp256k1.prime := 7

/-! ### ECDSA verification -/

/-- Decode 32 big-endian bytes into a `Nat`. -/
def bytesToNatBE (bs : Array (BitVec 8)) : Nat :=
  bs.foldl (fun acc b => acc * 256 + b.toNat) 0

/-- Modular exponentiation mod `orderN` via right-to-left binary expansion. -/
...
/-- Inverse mod `orderN` via Fermat (n is prime). -/
...
FIPS 186-4 §6.4.2. The Noir backend already vets that `(qx, qy)` lies
on the curve when accepting a public key, so we do not re-check here. -/
def verifyBytes
  ...
  let qx : Fp Secp256k1.prime := ((bytesToNatBE pkX : Nat) : Fp _)
  let qy : Fp Secp256k1.prime := ((bytesToNatBE pkY : Nat) : Fp _)
  let zRed := z % orderN
  let sInv := orderInv s
  let u1 := (zRed * sInv) % orderN
  let u2 := (r * sInv) % orderN
  let G : Point (Fp Secp256k1.prime) := .affine Gx Gy
  let Q : Point (Fp Secp256k1.prime) := .affine qx qy
...
Reproducible via `scripts/secp256k1_ref.py` (RFC 6979 deterministic
ECDSA). Matching these vectors transitively certifies agreement with
