  ...
  | Lean.Expr.const n _ =>
    ...
    | ``Lampe.Expr.callBuiltin =>
      ...
      | Lean.Expr.const n _ =>
        ...
        | ``Lampe.Builtin.projectRef => return some (← ``(genericTotalPureBuiltin_intro (Builtin.projectRef _) rfl))
...
/--
Trait-helper definitions generated by `noir_trait_def`. The macro emits one auxiliary
definition per category and v4.29's `simp` won't unfold them on its own, leaving goals
like `«…».#output h![]` unresolved after `resolve_trait` and stalling the subsequent
`steps`. Unfolding by name suffix is a deliberate narrow workaround until we either
move past v4.29 (PR #13363 restores some of the lost iota) or rework `noir_trait_def`
to tag these `@[reducible]`. -/
private def isTraitHelper (e : Lean.Expr) : Bool :=
  if let .const n _ := e.getAppFn then
    let s := n.componentsRev.head?.map (·.toString) |>.getD ""
    s == "#output" || s == "#inputs"
      || s == "#genericKinds" || s == "#associatedTypesKinds"
  else false

/--
Force beta, iota (matcher), structure-projection reduction, and trait-helper unfolding
on an expression under `.all` transparency.

v4.29.0 (leanprover/lean4#12179, #12572) stopped `simp only []` and `isDefEq` from running
this combination on noir_def function bodies (`FunctionDecl.fn → Function.body → Lambda.body`)
and on the `match h![T] with | h![T] => …` pattern whose pattern variable shadows the
scrutinee. We drive the reduction explicitly here. Used by `simpOnlyGoal` and by the
`reduce_fn_body` tactic. PR #13363 in v4.30.0 restores the matcher iota allowlist; once we
bump past that this helper can disappear.
-/
private partial def reduceBetaIotaMatch (e : Lean.Expr) : Lean.MetaM Lean.Expr := do
  let step (e : Lean.Expr) : Lean.MetaM Lean.Expr := Lean.Meta.withTransparency .all do
    ...
    let e ← match ← Lean.Meta.reduceProj? e with
      | some e' => pure e'
      | none => pure e
    if isTraitHelper e then
      match ← Lean.Meta.unfoldDefinition? e with
      | some e' => return e'
      | none => return e
    else return e
  Lean.Meta.transform e
    (pre := fun e => do let e' ← step e; return if e' == e then .continue else .visit e')
    (post := fun e => do let e' ← step e; return .done e')
...
partial def simpOnlyGoal (goal : MVarId) : TacticM MVarId := do
  ...
  let goal := match r.1 with
    | some (_, g) => g
    | none => goal
  goal.withContext do
    let ty ← goal.getType
    let ty' ← reduceBetaIotaMatch ty
    if ty' == ty then return goal
    let newGoal ← Lean.Meta.mkFreshExprMVar ty'
    goal.assign newGoal
