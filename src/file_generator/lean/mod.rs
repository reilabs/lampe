...
impl FileGenerator {
    ...
    fn generate_extracted_file(
        ...
        // If the file exists, we can overwrite it under two conditions. The first is if
        // it is a file generated by Lampe (as users should never modify these), while
        // the second is when the CLI is being run with the force-overwrite flag set.
        if output_file.exists() {
            let file_contents = fs::read_to_string(&output_file)?;

            if !file_contents.contains(LAMPE_GENERATED_COMMENT) {
                if self.overwrite {
                    eprintln!("Overwrite flag set: overwriting {}", &output_file.display());
                } else {
                    eprintln!(
