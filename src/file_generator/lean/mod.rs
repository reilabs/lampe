...
macro_rules! overwrite_check {
    ($file_name:ident, $check:expr) => {
        if $file_name.exists() {
            let file_contents = fs::read_to_string(&$file_name)?;
            if !file_contents.contains(LAMPE_GENERATED_COMMENT) && !$check {
                eprintln!(
...
impl FileGenerator {
    ...
    fn generate_lib_file(&self) -> Result<(), Error> {
        ...
        // We don't write the Lampe generated comment into the root library, as users
        // may change this.
        writeln!(result)?;
        writeln!(
            result,
