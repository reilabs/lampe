...
impl Writer<'_> {
    ...
    pub fn write_expression(&mut self, expr: &Expression) {
        match expr {
            ...
            Expression::BuiltinCallRef(call_ref) => self.append_to_line(call_ref),
            Expression::Call(call) => self.write_call_expression(call),
            Expression::Cast(cast) => self.write_cast_expression(cast),
            Expression::Constructor(constructor) => self.write_constructor_expression(constructor),
            ...
            Expression::Skip => self.append_to_line("skip"),
    ...
    pub fn write_type_expression(&mut self, expr: &TypeExpr) {
        match expr {
            TypeExpr::Alias(data) | TypeExpr::Struct(data) => self.write_data_type_expression(data),
...
impl Writer<'_> {
    ...
    pub fn write_let(&mut self, lets: &LetStatement) {
        ...
        self.append_to_line(" : ");
        self.write_type_value(&lets.typ, false);
        self.append_to_line(" = ");
    ...
    pub fn write_lvalue(&mut self, lvalue: &LValue) {
        match lvalue {
            LValue::Ident(ident) => self.write_identifier(ident, false),
            LValue::MemberAccess { object, index, typ } => {
                self.append_to_line("(");
                self.append_to_line("Object.at");
                self.append_to_line("(");
    ...
    pub fn write_member_access(&mut self, member_access: &MemberAccess) {
        self.append_to_line("Struct.get");
        self.append_to_line("(");
    ...
    pub fn write_literal(&mut self, literal: &Literal) {
        match literal {
            Literal::Bool(b) => {
                if *b {
                    self.append_to_line("true");
                } else {
                    self.append_to_line("false");
                }
            }
            Literal::NumericLiteral(lit) => {
                self.append_to_line("(");
            ...
            Literal::Unit => self.append_to_line("Unit"),
        }
    }
    ...
    pub fn write_pattern(&mut self, pattern: &Pattern) {
        match pattern {
            Pattern::Identifier(ident) => self.write_identifier(ident, false),
    ...
    /// Directly writes the contents of the constructor expression into the
    /// builder.
    ...
    pub fn write_block_expression(&mut self, block: &Block) {
        if block.statements.is_empty() {
            if let Some(expr) = &block.expression {
                match expr.as_ref() {
                    Expression::Block(_) => self.write_expression(expr),
                    _ => {
    ...
    pub fn write_function_type_expression(&mut self, function: &FunctionTypeExpr) {
        ...
        self.append_to_line(") -> ");
        self.write_type_expression(&function.return_type);
    }
    ...
    pub fn write_cast_type_expression(&mut self, cast: &CastTypeExpr) {
        self.append_to_line("Cast<");
        self.write_type_expression(&cast.target);
        self.append_to_line(">");
    }
    ...
    pub fn write_builtin_type_expression(&mut self, builtin: &BuiltinTypeExpr) {
        let tag = match &builtin.tag {
            BuiltinTag::Array => "Array".to_string(),
            BuiltinTag::Bool => "Bool".to_string(),
            BuiltinTag::Field => "Field".to_string(),
            BuiltinTag::FmtString => "FmtString".to_string(),
            BuiltinTag::I(n) => format!("i{n}"),
            BuiltinTag::Quoted(t) => format!("Quoted<{t}>"),
            BuiltinTag::Reference(mutability) => match mutability {
            ...
            BuiltinTag::U(n) => format!("u{n}"),
            BuiltinTag::Unit => "Unit".to_string(),
        };
