...
impl LeanGenerator<'_, '_> {
    ...
    pub fn generate_member_access(&self, member: &HirMemberAccess) -> Expression {
        ...
        let field_order = match value_type {
            NoirType::DataType(struct_def, _) => self.get_field_index_map(&struct_def),
            _ => panic!("Attempted to access member on non-struct type {value_type:?}"),
        };
                    ...
                    let func_name = match &func_meta.self_type {
                        Some(self_type) => {
                            ...
                            if let TypeExpr::Struct(s) = maybe_self_type.expr {
                                ...
                                panic!("Non-struct self type in non-trait function")
                            }
                        }
                        None => name,
                    };
