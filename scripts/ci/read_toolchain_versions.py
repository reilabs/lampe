...
def read_rust_nightly(root: pathlib.Path) -> str:
    versions = read_toml(root / "scripts" / "ci" / "toolchain_versions.toml")
    return versions["rust_nightly"]
...
def main() -> None:
    ...
    rust_nightly = read_rust_nightly(root)
    ...
    write_output("rust_nightly", rust_nightly)
