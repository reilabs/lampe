...
def build_lake(lampe_dir):
    ...
    if "CI" in env:
        env.pop("CI", None)
        env["LEAN_OPTIONS"] = "-Dsays.no_verify_in_CI=true"
        env["LEAN_CC"] = str(project_root / "scripts" / "lean_cc_wrapper.py")
...
def run_test(dir_path):
    ...
    for lampe_dir in lampe_dirs:
        ...
        if lakefile_path.exists():
            ...
            set_toml_lean_option(lakefile_path, "says.no_verify_in_CI", True)
            ...
            if not packages_link.exists():
                packages_link.symlink_to(os.path.relpath(packages_root, lake_dir), target_is_directory=True)
