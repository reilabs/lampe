...
match what F.2.1 publishes. (Our wrapper additionally appends a 16-byte
...
# Sanity: verify our toolchain reproduces NIST §F.2.1 (the raw,
# unpadded portion). Since we pad with PKCS#7 the output will be one
# block longer; truncating that to the first 64 bytes must equal
# NIST_CT_F21_RAW.
...
def emit_vector(name: str, key: bytes, iv: bytes, pt: bytes, comment: str = ""):
    ct = aes128_cbc_pkcs7(key, iv, pt)
    n_in = len(pt)
    n_out = len(ct)
    if comment:
        print(f"-- {comment}")
    print(f"private def {name}Key : List.Vector (BitVec 8) 16 := ⟨{fmt_bytes(key)}, by decide⟩")
    print(f"private def {name}Iv : List.Vector (BitVec 8) 16 := ⟨{fmt_bytes(iv)}, by decide⟩")
    print(f"private def {name}In : List.Vector (BitVec 8) {n_in} := ⟨{fmt_bytes(pt)}, by decide⟩")
    print(f"private def {name}Out : List.Vector (BitVec 8) {n_out} := ⟨{fmt_bytes(ct)}, by decide⟩")
    print(f"theorem aes128_{name}_correct :")
    print(f"    (aes128CbcEncryptPkcs7 {name}Key {name}Iv {name}In).toList = {name}Out.toList := by native_decide")
    print()
