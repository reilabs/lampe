...
Emits Lean test vectors comparing
  Lampe.Crypto.Secp256k1.verifyBytes pkX pkY sig msgHash
to the expected verification result (`true` for valid signatures,
`false` for tampered ones), checked via `native_decide`. Since
Barretenberg's `__ecdsa_secp256k1` foreign call runs the same FIPS
186-4 verification algorithm, agreement with the Python reference
transitively certifies agreement with Barretenberg.

Usage:
    python3 scripts/gen/secp256k1_ref.py <path/to/Ecdsa.lean>
"""
...
def main():
    if len(sys.argv) != 2:
        print("usage: secp256k1_ref.py <path/to/Ecdsa.lean>", file=sys.stderr)
        sys.exit(1)
